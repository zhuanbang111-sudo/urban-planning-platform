  ...
  <div class="space-y-6">
    ...
    <div class="border-b border-gray-200 pb-4">
      ...
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="w-full md:w-64">
          <label class="block text-xs font-semibold text-gray-500 mb-1">政策类型过滤</label>
          <select 
            v-model="selectedCategory"
            class="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg shadow-sm text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          >
    ...
    <div v-if="isLoading" class="py-12 flex justify-center items-center">
      <div class="flex flex-col items-center gap-3">
        ...
        <span class="text-sm text-gray-500">正在调阅政策规范列表...</span>
      </div>
    </div>
    ...
    <div v-if="paymentModal.show" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 relative animate-fade-in border border-gray-100">
        ...
        <div class="text-center">
          <div class="w-12 h-12 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl">
            💎
          </div>
          ...
          <p class="text-xs text-gray-400 mt-1">
            需要获取当前资源的 <span class="font-bold text-blue-600">{{ paymentModal.type === 'view' ? '在线预览' : '物理下载' }}</span> 授权
          </p>

          <!-- 价格展示 -->
          <div class="my-5 bg-blue-50/50 py-3 rounded-xl border border-blue-100">
...
const fetchResources = async () => {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/resources?module=policy`)
    if (res.ok) {
      const data = await res.json()
