  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-1">上传、修改及下架规范资料与技术图集资源文件</p>
      </div>
      <div>
        <button 
          @click="openAddModal"
    ...
    <div v-if="loading" class="flex justify-center items-center py-20 bg-white rounded-xl shadow border border-gray-100">
      ...
      <span class="ml-3 text-gray-500 text-sm">正在加载资料数据...</span>
    </div>
    ...
    <div v-else-if="resources.length === 0" class="bg-white rounded-xl shadow border border-gray-100 p-20 text-center text-gray-400">
      ...
      <p class="text-base font-medium">暂无相关的资料数据，请先点击右上角上传资料文件</p>
    </div>
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-gray-50 border-b border-gray-100">
              ...
              <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">资料名称</th>
    ...
    <div v-if="showFormModal" class="fixed inset-0 bg-black/60 flex justify-center items-center z-50 p-4 overflow-y-auto">
      <div class="bg-white rounded-2xl max-w-xl w-full shadow-2xl border border-gray-100 p-6 space-y-4">
        <div class="flex justify-between items-center border-b pb-3">
          <h3 class="text-lg font-bold text-gray-800">{{ isEdit ? '✏️ 编辑资料内容' : '📤 上传新资料文件' }}</h3>
        ...
        <form @submit.prevent="submitForm" class="space-y-4">
          <!-- 标题 -->
          <div>
            <label class="block text-xs font-bold text-gray-700 mb-1">资料标题 *</label>
            <input v-model="form.title" type="text" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="请输入图集规范资料名称">
          </div>
          ...
          <div>
            ...
            <textarea v-model="form.description" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="请简要描述本份规范图集的核心指引内容"></textarea>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="form.category" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="如：规划设计图集、道路排水规范">
            </div>
            <div>
...
const fetchResources = async () => {
  ...
  try {
    ...
    alert('无法加载资料列表，错误提示: ' + (err.response?.data?.message || err.message))
  } finally {
    loading.value = false
  }
}
