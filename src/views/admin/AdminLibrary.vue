<!-- 保存路径: src/views/admin/AdminLibrary.vue -->
<template>
  <div class="space-y-6">
    <!-- 顶部状态栏 -->
    <div class="bg-white rounded-xl border border-gray-150 p-6 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-1">
          管理国家、行业、地方级规范设计规程资料。支持设定特定的等级和下载费率。
        </p>
      </div>
      <button 
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-gray-100 text-sm">
            <tr v-if="loading" class="text-center">
              <td colspan="8" class="py-12 text-gray-400 font-medium">正在读取规范文件物理存储索引...</td>
            </tr>
            <tr v-else-if="filteredResources.length === 0" class="text-center">
              <td colspan="8" class="py-12 text-gray-400 font-medium">暂无匹配的规范文献，请点击右上角开始录入。</td>
            </tr>
    ...
    <div v-if="showFormModal" class="fixed inset-0 z-40 bg-gray-950/50 backdrop-blur-xs flex items-center justify-center p-4">
      <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-fade-in">
        <div class="bg-blue-900 text-white px-6 py-4 flex justify-between items-center">
          <h3 class="font-bold text-lg">{{ isEditMode ? '🛠 编辑规程卡片属性' : '📤 上传全新规程文献' }}</h3>
        ...
        <form @submit.prevent="submitForm" class="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          <div>
            ...
            <input v-model="form.title" type="text" required placeholder="如：给水排水管道工程施工及验收规范" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-900" />
          </div>

          <div>
            <label class="block text-xs font-bold text-gray-700 mb-1">规程/内容简析描述</label>
            <textarea v-model="form.description" rows="3" placeholder="适用规程条款及限制要素说明..." class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-900"></textarea>
          </div>

          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-bold text-gray-700 mb-1">二级分类</label>
              <input v-model="form.category" type="text" placeholder="如：国家标准" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-900" />
            </div>
            <div>
          ...
          <div>
            <label class="block text-xs font-bold text-gray-700 mb-1">实体规程文件 (PDF/Word) *</label>
            ...
            <p v-if="isEditMode" class="text-[10px] text-gray-400 text-center mt-1">留空则保持原规程文件不变</p>
          </div>
